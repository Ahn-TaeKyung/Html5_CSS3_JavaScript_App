...
bookForm.addEventListener("submit", function (event) {
    ...
    const bookData = {
        ...
        detailRequest:{
            ...
            pageCount: bookFormData.get("pageCount"),
        }
    };         
...
function validateBook(book) {// 필수 필드 검사
    ...
    if (!book.detailRequest.pageCount) {
        alert("페이지 수수를 입력해주세요.");
        return false;
    }
    // 일련번호 형식 검사 (예: 영문과 숫자 조합)
    const isbnPattern = /^[B-Za-z0-9]+$/;
    if (!isbnPattern.test(book.isbn)) {
        alert("일련번호는 영문과 숫자만 입력 가능합니다.");
        return false;
    }
...
function editBook(bookId) {
    fetch(`${API_BASE_URL}/api/books/${bookId}`)
        ...
        .then((book) => {
            ...
            //버튼의 타이틀을 등록 => 수정으로 변경
            submitButton.textContent = "도서서 수정";
            //취소 버튼을 활성화
